...
# Standartları bellekte tut (basit çözüm)
standards_store = {}
...
def parse_excel(file):
    ...
    for i in range(header_row + 1, len(df)):
        ...
        records.append({'no': no, 'name': name if name != 'nan' else '', 'tarih': tarih, 'status': 'pending', 'note': ''})
...
def export():
    ...
    rows = [['Standart No', 'Standart Adı', 'Listedeki Tarih', 'Durum', 'Not']]
    for s in records:
        durum = {'current': 'Güncel', 'update': 'Güncelleme Var', 'pending': 'Kontrol Edilmedi', 'opened': 'Kontrol Edilmedi'}.get(s['status'], '')
        rows.append([s['no'], s['name'], s['tarih'], durum, s['note']])
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        for row in ws.iter_rows(min_row=2):
            durum_cell = row[3]
            if durum_cell.value == 'Güncelleme Var':
                for cell in row:
                    cell.fill = red_fill
            elif durum_cell.value == 'Güncel':
                for cell in row:
